...
class AnalyticsService:
    ...
    async def get_overall_stats(db: AsyncSession, user_id: int) -> dict[str, Any]:
        ...
        vpip_hands = sum(1 for h in hands if h.action_taken and h.action_taken != "fold")
    ...
    async def get_action_stats(db: AsyncSession, user_id: int) -> dict[str, Any]:
        ...
        for action, stats in actions.items():
            win_rate = (stats["wins"] / stats["total"] * 100) if stats["total"] > 0 else 0.0
            distribution = (stats["total"] / total_actions * 100) if total_actions > 0 else 0.0
    ...
    async def get_win_rate_over_time(db: AsyncSession, user_id: int) -> dict[str, Any]:
        ...
        for hand in hands:
            ...
            win_rate = (cumulative_wins / cumulative_total * 100) if cumulative_total > 0 else 0.0
    ...
    @staticmethod
    async def get_playing_style_profile(db: AsyncSession, user_id: int) -> dict[str, Any]:
